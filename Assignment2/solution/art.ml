...
let rec build2 (rand,depth) = 
    if depth = 0 then
        ...
        match rand(0, 100) mod 9 with
          ...
          | 7 -> buildFloor(build(rand, depth-1), build(rand, depth-1))
          | _ -> buildSqrt(build(rand, depth-1), build(rand, depth-1), build(rand, depth-1))
